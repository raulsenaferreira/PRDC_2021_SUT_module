...
def get_technique_params(technique):
	# Default
	#passar pra dentro do get_experiment_params 
	PARAMS = {}
	if 'sgd' == technique:
...
def get_experiment_params(technique, dataset_name):
	...
	if technique in 'oob':
		...
		PARAMS.update({'noiseMagnitude': 0.0014}) # gtsrb = 0.0025; cifar-10 = 0.0014
		PARAMS.update({'temper': 1000})
		# it is the rounding value of the min confidence threshold rounded in 4 decimals (0.10069 = 0.1007) 
		PARAMS.update({'threshold': 0.1007}) # gtsrb = 0.0237; cifar-10 = 0.1007
		PARAMS.update({'OOD_approach': 'temperature'})
		PARAMS.update({'use_alternative_monitor': False})# True = label -> act func; False = label -> act func if label == predicted
		PARAMS.update({'use_scaler': False})
		PARAMS.update({'grid_search': False})

	elif technique == 'alooc':
		PARAMS.update({'backend': 'tensorflow'}) 
		
		PARAMS.update({'OOD_approach': 'adversarial'})
		PARAMS.update({'use_alternative_monitor': False}) # True => based just on label; False => based on predicted == label
		PARAMS.update({'use_scaler': False})
		PARAMS.update({'grid_search': False})
