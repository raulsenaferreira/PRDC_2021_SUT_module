...
from src.utils import scgan_kh_tools


class ALOCC_Model():
    def __init__(self, input_height=28, input_width=28, output_height=28, output_width=28, is_training=True,
               z_dim=100, gf_dim=16, df_dim=16, log_dir='log', sample_dir='sample', r_alpha = 0.2):
        """
    ...
    def build_generator(self, input_shape):
        ...
        x = UpSampling2D((2, 2))(x)
        x = Conv2D(1, kernel_size=5, activation='sigmoid', padding='same')(x)
    ...
    def train(self, X_train, y_train, attention_label, epochs, batch_size, sample_interval, models_folder, model_name):
        plot_epochs = []
        plot_g_recon_losses = []
        c_dim = 1
        counter = 1
        ...
        specific_idx = np.where(y_train == attention_label)[0]
        self.data = X_train[specific_idx].reshape(-1, 28, 28, 1)
        sample_w_noise = scgan_kh_tools.get_noisy_data(self.data)
        ...
        #imageio.imwrite('./{}/train_input_samples.jpg'.format(self.sample_dir), scgan_utils.montage(sample_inputs[:,:,:,0]))
        ...
        for epoch in range(epochs):
            ...
            self.save(models_folder, model_name, epoch)
        ...
        plt.savefig(os.path.join('log', 'plot_g_recon_losses.png'))
    ...
    def save(self, folder, name, step='model'):
        """Helper method to save model weights.
        Arguments:
            step {[type]} -- [description]
        """
        os.makedirs(folder, exist_ok=True)
        name = name+'_{}.h5'.format(step)
        self.adversarial_model.save_weights(os.path.join(folder, name))    